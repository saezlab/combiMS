...
# Original file : normaliseSimp.R
# 
# modified by Melanie Rinas
# all modifications are marked with
# # MRmodified


normaliseSimp_MR=function(taula,HillCoef=2){
   ...
   for (i in 1:length(names(taula2))){
      #colnames(taula2)[i]=strsplit(names(taula2[i]),"\\.")[[1]][2]                  # original      
      colnames(taula2)[i]=strsplit(names(taula2[i]),":")[[1]][2]                     # MRmodified
   }
   ...
   # MRmodified: deleted loop as not necessary (does not go over index i)
   
   # for (i in 1:numProt){
   #    #noiseProt=intersect(which(pvalueGreater>0.05),which(pvalueLess>0.05))      # original   
   #    noiseProt=intersect(which(pvalueGreater>=0.05),which(pvalueLess>=0.05))     # MRmodified: inserted 2 times  =
   #    
   # }
   ...
   if (length(positiveProt)>0){
      ...
      for(i in 1:numExp){
         for (j in 1:length(positiveProt)){
            ...
            if (taula2[i,positiveProt[j]]>=(-1) && taula2[i,positiveProt[j]]<0 && !is.na(taula2[i,positiveProt[j]])){  # MRmodified: inserted =
      ...
      for(i in 1:numExp){
         for (j in 1:length(negativeProt)){
           # if (taula2[i,negativeProt[j]]>0 && taula2[i,negativeProt[j]]<1 && !is.na(taula2[i,negativeProt[j]])){
             if (taula2[i,negativeProt[j]]>0 && taula2[i,negativeProt[j]]<=1 && !is.na(taula2[i,negativeProt[j]])){   # MRmodified: inserted =
            ...
            } else if (taula2[i,negativeProt[j]]!=0 && !is.na(taula2[i,negativeProt[j]])){                           # MRmodified: inserted !=
   ...
   if(length(noiseProt)>0){
      for(i in 1:numExp){
         for (j in 1:length(noiseProt)){
            ...
            if(taula2[i,noiseProt[j]]>=(-1) && taula2[i,noiseProt[j]]<=1 && !is.na(taula2[i,noiseProt[j]])){                  # MRmodified: inserted =
