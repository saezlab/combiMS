...
# ERROR correction 
# 
# by initiating the missing object
...
# 
# SUGGESTION
# keep also graphModel_original with negative interactions
...
network2graph_MR<-function(phenotypeNw,mode='mean'){
   ...
   if(mode=='median'){
      ...
      linkActivityThreshold=0.5                                                     # MR inserted as missing (use same linkActivityThreshold as in the previously needed script phenotypeNetwork.R)
   ...
   graphModel_original=sif2graph(tempSIF)                                                    # MR modified            
   
   # replace inhibition by activation, as the graph only accepts positive interactions
   tempSIF[,2]=abs(as.numeric(tempSIF[,2]))                                                
   graphModel_inhibition_replaced_by_activation=sif2graph(tempSIF)                           # MR modified       
   
   ModelGraph=list("model"=modelPhenotype,
                   "graph_original"=graphModel_original,
                   "graph_inhibition_replaced_by_activation"=graphModel_inhibition_replaced_by_activation,
                   "network"=phenotypeNw,"cutNetwork"=cutNw)
